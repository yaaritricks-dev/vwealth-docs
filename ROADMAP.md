...
- Design review complete hai; naye unrequested design rounds nahi. Android Steps 2/3, Step 4 aur Batch A complete aur owner phone-approved hain (2026-09-17; latest accepted APK 0.3.1). Agla kaam Batch B: Reports, Subscriptions/Recurring, Targets/Goals. Remaining native spike/security gates, approved scope aur estimates unchanged hain.
...
- Steps 2/3 complete and owner phone-approved: native foundation, design system, five-tab navigation, Home/shared Quick Add, accepted spacing and accent-tint feedback. Runtime now uses persisted local data; the old in-memory-only status is historical.
- Step 4 COMPLETE, owner signed off: local persistence, calendar date picker, readable dates, Ledger search/filters. Batch A COMPLETE, owner signed off: Accounts/opening balances/create-edit/reconcile/transfer/crypto conversion/crypto detail; Liabilities including principal-only payoff with visible assumption; shared entity detail layout; live net worth. Basic report-to-entity links exist; full Reports work is next in Batch B.
- Owner tested APK 0.3.1 on the phone and accepted partial net worth plus `2 accounts not set`, group-wise missing counts, assets-minus-liabilities math, liability calendar dates (no free-text `Test` date), Audit icon, entity-detail tabs/filters/average/largest, and persistence. These are owner-reported phone checks, not direct Codex device inspection.
- OWNER-APPROVED PROTOTYPE DEVIATION: Home delta is actual current net worth minus a real dated baseline, NEVER the prototype's income-minus-spend formula. Complete balance setup establishes the real baseline; do not invent history or fill unobserved dates. Hide the entire delta row while current balances or comparison history are insufficient. Approved history copy: `Net worth change will appear once there's earlier history to compare`. Money in/out remains separate cashflow.
- OWNER-APPROVED OFFLINE CRYPTO DEVIATION: `Manual rate · not live`; refresh says `Market refresh unavailable offline. Saved rate and timestamp unchanged.` Retain the original saved quote/rate timestamp; no fabricated live price, network call or requested permission.
- Partial-state rule: show the known total plus the scoped `N accounts not set` / `N liabilities not set` count (singular for one). Unknown amounts stay unknown/null, never silently become zero. Entirely unknown groups show `Not set`; complete groups have no missing-count message. Mixed Home copy is ONE line, e.g. `2 accounts · 1 liability not set`. An empty liability list has no recorded debt, distinct from an existing liability with unknown outstanding. Partial totals do not fabricate complete-worth snapshots/deltas.
- Mandatory before EVERY APK: real-user walkthrough from a fresh empty app, create 3–4 accounts with mixed set/blank balances, add a liability, enter current/past-date entries, transfer/reconcile, stop/reopen and check truthfulness/partial states and persistence at every stage. Pre-seeded financial fixtures do not substitute for that journey. Current automated coverage uses real MainActivity UI writes and cold Activity/store relaunch on JVM; report actual phone OS force-stop separately. Also run partial-state light/dark screenshots and two consecutive full suites.
- Verified 0.3.1 delivery: both final suites passed 1,079/1,079 tests with zero failures/errors/skips and 1,026 unchanged screenshot comparisons each; 90 partial-state screenshot cases cover five widths and both themes. Signed APK verified, zero requested permissions; lint zero errors. JVM and owner phone evidence remain distinct; OS/security/phone-only gates are not waived.
- Next: Batch B — Reports, Subscriptions/Recurring, Targets/Goals. This is a progress update only: the approved feature inventory, scope and estimates below are unchanged. Encryption/biometric/sync and other later gates are not marked complete.
